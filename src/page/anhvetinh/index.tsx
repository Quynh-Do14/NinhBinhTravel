...
const ListAnhVeTinhManagement = () => {
    ...
    const onGetListAsync = async ({ name = "", size = pageSize, page = currentPage }) => {
        ...
        try {
            await anhvetinhService.GetAnhvetinh(
                param,
                setLoading
            ).then((res) => {
    ...
    const onDeleteAsync = async () => {
        try {
            await anhvetinhService.DeleteAnhvetinh(
                idSelected,
                setLoading
            ).then((res) => {
                if (res) {
    ...
    const onNavigate = (id: any) => {
        navigate(`${(ROUTE_PATH.VIEW_ANHVETINH_MANAGEMENT).replace(`${Constants.UseParams.Id}`, "")}${id}`);
    }
