...
let timeout: any;
const ListDanhMucLopDuLieuManagement = () => {
  ...
  const onGetListAsync = async ({
    ...
    try {
      await danhmucService.GetDanhmuc(param, setLoading).then((res) => {
        setListResponse(res.data.danhmuclopbandos);
  ...
  const onDeleteAsync = async () => {
    try {
      await danhmucService.DeleteDanhmuc(idSelected, setLoading).then((res) => {
        if (res) {
          setIsDeleteModal(false);
          onSearch().then(() => {});
        }
  ...
  const onNavigate = (id: any) => {
    navigate(
      `${ROUTE_PATH.VIEW_DANH_MUC_LOP_DU_LIEU_MANAGEMENT.replace(
        ...
        <div className="content">
          <Row gutter={[20, 20]}>
            ...
            <Col xs={24} sm={18} className="flex justify-end">
              <ButtonCommon
                classColor={"red"}
                onClick={() =>
                  navigate(ROUTE_PATH.ADD_DANH_MUC_LOP_DU_LIEU_MANAGEMENT)
                }
                title={"Thêm mới"}
              />
            </Col>
          ...
          <div className="table-container">
            ...
            >
              ...
              <Column
                title={
                  <TitleTableCommon title="Tên danh mục" width={"150px"} />
                }
                key="tendanhmuclopbando"
                dataIndex="tendanhmuclopbando"
                sorter={(a, b) =>
                  a.tendanhmuclopbando.localeCompare(b.tendanhmuclopbando)
                }
                defaultSortOrder="ascend" // tùy chọn, nếu bạn muốn mặc định sắp xếp A → Z
              />
              <Column
                title="Thao tác"
                fixed="right"
                ...
                render={(action, record: any) => (
                  <ActionCommon
                    onClickDetail={() => onNavigate(record.iddanhmuclopbando)}
                    onClickDelete={() =>
                      onOpenModalDelete(record.iddanhmuclopbando)
                    }
                  />
                )}
              />
...
export default ListDanhMucLopDuLieuManagement;
