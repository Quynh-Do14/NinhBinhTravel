...
const ListAPIManagement = () => {
    ...
    const onGetListAsync = async ({ name = "", size = pageSize, page = currentPage }) => {
        ...
        try {
            await danhMucAPIService.GetDanhMucAPI(
                param,
                setLoading
            ).then((res) => {
                setListResponse(res.data.apis)
    ...
    const onDeleteAsync = async () => {
        try {
            await danhMucAPIService.DeleteDanhMucAPI(
                idSelected,
                setLoading
            ).then((res) => {
                if (res) {
    ...
    const onNavigate = (id: any) => {
        navigate(`${(ROUTE_PATH.VIEW_API_MANAGEMENT).replace(`${Constants.UseParams.Id}`, "")}${id}`);
    }
                ...
                <div className="content">
                    <Row gutter={[20, 20]}>
                        ...
                        <Col xs={24} sm={18} className='flex justify-end'>
                            <ButtonCommon
                                classColor={'red'}
                                onClick={() => navigate(ROUTE_PATH.ADD_API_MANAGEMENT)}
                                title={'Thêm mới'}
                            />
                        </Col>
                    </Row>
                        ...
                        >
                            ...
                            <Column
                                ...
                                render={(action, record: any) => (
                                    <ActionCommon
                                        onClickDetail={() => onNavigate(record.idapi)}
                                        onClickDelete={() => onOpenModalDelete(record.idapi)}
                    ...
                    <DialogConfirmCommon
                        message={"Bạn có muốn xóa API này ra khỏi hệ thống"}
                        titleCancel={"Bỏ qua"}
                        titleOk={"Xóa API"}
...
export default ListAPIManagement